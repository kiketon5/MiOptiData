...
const PDFExport = ({ onClose }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      yPosition += 5;
      pdf.text(`Report Date: ${new Date(selectedDate).toLocaleDateString()}`, 20, yPosition);
      yPosition += 15;
  ...
  const loadPrescriptions = async () => {
    const { data, error } = await supabase
      ...
      .eq('prescription_date', selectedDate) // exact date
  ...
  const loadVisualTests = async () => {
    const { data, error } = await supabase
      ...
      .eq('user_id', user.id)
      .eq('profile_id', profileId)
      .eq('test_date', selectedDate) // exact date
  ...
  const loadSymptoms = async () => {
    const { data, error } = await supabase
      ...
      .eq('user_id', user.id)
      .eq('profile_id', profileId)
      .eq('symptom_date', selectedDate) // exact date
  ...
  const loadPressureMeasurements = async () => {
    const { data, error } = await supabase
      ...
      .eq('user_id', user.id)
      .eq('profile_id', profileId)
      .eq('measurement_date', selectedDate) // exact date
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div className="space-y-4">
          ...
          {/* 🔹 Cambié el rango por una sola fecha */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Select Date
            </label>

  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (reportType === "complete" || reportType === "summary") {
        ...
        pdf.line(15, yPosition - 10, pageWidth - 15, yPosition);
        yPosition += 10;
      }
      ...
      if (
        ...
        // Rectángulo de fondo para separar el bloque
        pdf.setFillColor(240, 240, 240); // color gris claro
        pdf.roundedRect(15, yPosition - 2, pageWidth - 30, 55, 3, 3, "F"); // ajusta altura si es necesario
        ...
        // Tipo de test
        pdf.setFont(undefined, "bold");
        pdf.text("Test Type:", xText, yText);
        pdf.setFont(undefined, "normal");
        pdf.text(latestTest.test_type.toUpperCase(), xText + 35, yText);

        yText += 7;

        // Corrección
        pdf.setFont(undefined, "bold");
        pdf.text("With Correction:", xText, yText);
        pdf.setFont(undefined, "normal");
        pdf.text(latestTest.with_correction ? "Yes" : "No", xText + 45, yText);

        yText += 10;

        // Resultados por ojo
        pdf.setFont(undefined, "bold");
        pdf.text("Right Eye (OD):", xText, yText);
        pdf.setFont(undefined, "normal");
        pdf.text(latestTest.od_result || "Not tested", xText + 40, yText);

        yText += 7;

        pdf.setFont(undefined, "bold");
        pdf.text("Left Eye (OS):", xText, yText);
        pdf.setFont(undefined, "normal");
        pdf.text(latestTest.os_result || "Not tested", xText + 40, yText);

        yText += 7;

        pdf.setFont(undefined, "bold");
        pdf.text("Both Eyes:", xText, yText);
        pdf.setFont(undefined, "normal");
        pdf.text(
        ...
        yPosition = yText + 10; // actualizar la posición para el siguiente bloque
      }
      ...
      // Save PDF
      const fileName = `eye-health-report-${profileData.name.replace(
        /\s+/g,
        "-"

    ...
    switch (filter) {
      case "active":
        filtered = filtered.filter(
          (reminder) => reminder.is_active && !reminder.is_completed
        );
        break;
      case "inactive":
      ...
      {filteredReminders.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredReminders.map((reminder) => (
            <div
              key={reminder.id}
              className={`bg-white rounded-lg shadow-md p-6 border-l-4 ${getReminderStatusColor(
                reminder
              )}`}
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {isUpcoming(reminder.reminder_date) &&
                      reminder.is_active &&
                      !reminder.is_completed &&
                      !isToday(reminder.reminder_date) && (
            ...
            <div className="text-2xl font-bold text-red-600">
              {
                reminders.filter(
                  (r) =>
                    r.is_active && !r.is_completed && isPast(r.reminder_date)
                ).length
              }
            </div>
